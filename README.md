...
A processor's first parameter is always the `card`; a plain `dict` representing a card's fields. Any parameters after that are automatically picked up from your function's signature and forwarded from the CLI arguments passed via `--args key=value`. A parameter with no default is required while one with a default is optional.
...
For example, you might have the same pipeline for different languages, so you would pass in `--args lang=german` or `--args lang=french`, and hanky calls your processor under the hood like so:
